...
// Interrupt on Receive time-out error
void UART2Interrupt (void){
	...
	// Clear interrupt flag for UART2_RTIM at bit6 
	UART2_ICR_R |= 0x40;		// 0100_0000 Set as 1 to clear	
	
	// Enable interrupt on UART2_RTIM at bit6 
	UART2_IM_R |= 0x40;			// 0100_0000 
...
unsigned char UART2Rx(void)
{
	while((UART2_FR_R & UART_FR_RX_FE)!=0 & flag!=1);
...
{	
	...
	while (flag != 1) { //char_ != '\0' // (UART2_FR_R & UART_FR_RX_FE)!=0
			UART0Tx(UART2Rx());
	}
	flag = 0;
}
