...
void UART2Interrupt (void){
	...
	/* Configure UART2 as edge sensitive,
	* not both edges,
	* falling edge */
	// Interrupt at 0x0 at bit5:3 RX FIFO = 1/8 full
	UART2_IFLS_R = (UART2_IFLS_R&0xFFFFFC7F); // C7 -> 1100_0111
	
	/* Clear interrupt flag for UART2_RX at bit4  */
	UART2_ICR_R |= 0x10;		// Set as 1 to clear	
	
	/* Enable interrupt on UART2_RX at bit4 */
	UART2_IM_R |= 0x10;	
