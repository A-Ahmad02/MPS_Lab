...
#define		TIM2_INT_CLR            0x400		// Interrupt GPTM Timer B
...
{
	TIMER1->TBMATCHR = (PERIOD*count/LEN); // duty cycle as ILR/4
	count += 1;
	if (count == LEN){
		count = 0;
	}
...
{
	TIMER0->TBMATCHR = (PERIOD*(LEN-count))/LEN; // 75% duty cycle as ILR/4
	count += 1;
	if (count == LEN){
		count = 0;
	}
